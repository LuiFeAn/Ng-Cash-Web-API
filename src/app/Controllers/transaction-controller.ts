...
class TransactionController {
    ...
    async create(request: Request, response: Response){
        ...
        if(!toUser) transactionErrors.push({reason:'O usuário para o qual receberá a transação não foi específicado'});
        if(!value) transactionErrors.push({reason:'O valor da transação não foi específicado'});

        if(transactionErrors.length > 0) return response.status(404).json( {error: transactionErrors} );
        ...
        if(fromUserAccount?.balance && fromUserAccount.balance < value){
            return response.status(401).json({ error: 'Você não possui saldo o suficiente para realizar essa transação '});
        }

        if(username === toUser) return response.status(401).json( {error: 'Você não pode realizar uma transação para si mesmo '} );

        const userRepository = AppDataSource.getRepository(User);

        const toUserTransaction = await userRepository.findOne({ where: { username: toUser}});
        if(!toUserTransaction) return response.status(404).json({ error: 'Usuário não existente !'});
        ...
        const transaction = transactionRepository.create({debitedAccountId:fromUserAccount?.id,creditedAccountId:toUserAccount?.id, value});
